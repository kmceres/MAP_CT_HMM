...
#load data
sequence, tau  =ghmm.read_file('hmm_ready_data.csv')
...
for i in range(n_iterations):
    ...
    #prepare train and test sets
        ...
        for key in cowid_sample:
            if key in train_set.keys():
                newid = key + str(int(time.time()))
                train_set[newid] = sequence[key]
                tau_train[newid] = tau[key]
            else:
...
mean_ci_df3.to_csv(path_or_buf = "mean_ci3_"+str(seed)+".csv", index = True)
